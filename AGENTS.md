...
- **Tech stack**: Markdown + JSON config (marketplace + plugin manifests); Bash for hooks; the Codex plugin format is the only deployment target — no separate runtime, no package registry
...
- **Compatibility**: Plugin layout must satisfy `/plugin marketplace add` and `/plugin install` flows on current Codex
...
|------------|---------|---------|-----------------|
| **Codex plugin spec** | current (live spec, docs at `code.Codex.com/docs/en/plugins-reference`) | The only deployment target | The PROJECT explicitly fixes this: `/plugin marketplace add` + `/plugin install` flows. No alternative runtime exists for shipping into Codex. |
| **`.Codex-plugin/marketplace.json`** | schema v1 (live) | Marketplace catalog at repo root | Required exact location; this is what `/plugin marketplace add owner/repo` looks for. |
| **`.Codex-plugin/plugin.json`** | schema v1 (live) | Per-plugin manifest | Only `name` is strictly required; everything else (`version`, `description`, `author`, component paths) is opt-in. |
...
| **OpenAI `codex` CLI** | `codex exec` mode (current; non-interactive subcommand is GA in 2026) | Independent reviewer for research/plan/code validation | PROJECT mandates codex (no Codex fallback) for cross-model review. `codex exec` is purpose-built for scripted invocation. |
...
|------|---------|-------------|
| **`Codex plugin validate`** | Validates `plugin.json`, `marketplace.json`, frontmatter, hooks JSON | Run on **every** commit that touches a manifest. PROJECT constraint MKT-06 requires it. |
...
| Tool | Purpose | Notes |
|------|---------|-------|
| **`Codex --plugin-dir ./plugins/zapili`** | Load the local plugin without publishing | Primary dev loop. `/reload-plugins` picks up edits without restart. |
| **`Codex plugin validate ./plugins/zapili`** | Validates plugin + every component file | Run before commit; pre-commit-style discipline without a hook. |
| **`Codex plugin validate . --strict`** | Treats warnings (e.g. misspelled fields) as errors | Use at marketplace root before releasing. |
...
### `.Codex-plugin/marketplace.json` (at `oplya` repo root)
...
### `plugins/zapili/.Codex-plugin/plugin.json`
...
- `${CLAUDE_PLUGIN_ROOT}` is mandatory: plugins are copied to `~/.Codex/plugins/cache/...` at install time, so relative paths must go through this variable.
...
- `cat >/dev/null` drains the JSON payload Codex writes to stdin; if you skip this, hooks can hang or get SIGPIPE on later writes.
...
- Plugin agents **cannot** use `hooks`, `mcpServers`, or `permissionMode` (security restriction). If you need any of those, copy the agent into `.Codex/agents/` instead. For `zapili` this is fine; none of the three subagents needs them.
...
- Add a one-liner `.git/hooks/pre-commit` that runs `Codex plugin validate . --strict` and `Codex plugin validate ./plugins/zapili --strict`.
...
- Use `${CLAUDE_PLUGIN_DATA}` (resolves to `~/.Codex/plugins/data/<plugin-id>/`). Survives updates. `.zapili/state.json` belongs to the task, not the plugin install, so it stays where it is.
...
|-------|-----|-------------|
| Putting `commands/`, `agents/`, `hooks/` **inside** `.Codex-plugin/` | The loader will silently not find them — `.Codex-plugin/` is only for `plugin.json`. This is the #1 cited mistake in the official docs. | Plugin-root level (siblings of `.Codex-plugin/`). |
...
| Hardcoded absolute paths in hook commands | Plugins are copied to `~/.Codex/plugins/cache/<...>/<sha>/`; absolute paths break instantly. | `${CLAUDE_PLUGIN_ROOT}` (always wrap in quotes: `"${CLAUDE_PLUGIN_ROOT}"`). |
...
| `Task` tool references in new agent files | Renamed to `Agent` in Codex v2.1.63. Old `Task(...)` still works as an alias but is deprecated. | Write `Agent(zapili-researcher, …)` in `allowed-tools`. |
...
|-------|----------|-------|
| `SessionStart` (matcher: `startup`) | ZAP-02: verify `codex` CLI is installed | Fires once when a fresh Codex session starts. Use `matcher: "startup\|resume"` if you also want to verify on resume; usually overkill. Exit code 2 surfaces stderr as a blocking error. |
...
|------|-----------------|-------|
| Marketplace schema (`marketplace.json` fields used above) | Codex current | All fields used (`name`, `owner`, `metadata.pluginRoot`, `plugins[*].source`, `plugins[*].keywords`, `plugins[*].category`) are stable / non-experimental. |
| Plugin manifest schema (`plugin.json` fields used above) | Codex current | `displayName` requires v2.1.143+; if you want to support older clients, drop `displayName` and rely on `name`. |
...
## Sources
- **Plugins overview** — https://code.Codex.com/docs/en/plugins — directory layout, manifest basics, `--plugin-dir`/`--plugin-url`, hooks/migration from `.Codex/`.
- **Plugins reference** — https://code.Codex.com/docs/en/plugins-reference — full `plugin.json` schema, all component path fields, hook event table, `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` / `${CLAUDE_PROJECT_DIR}`, caching/symlink behavior, version-resolution rules.
- **Plugin marketplaces** — https://code.Codex.com/docs/en/plugin-marketplaces — `marketplace.json` schema, all source types (`github`, `url`, `git-subdir`, `npm`, relative path), strict mode, validation, hosting on GitHub, `metadata.pluginRoot`, release channels.
...
- **Subagents** — https://code.Codex.com/docs/en/sub-agents — agent frontmatter schema (`name`, `description`, `model`, `effort`, `maxTurns`, `tools`, `disallowedTools`, `skills`, `memory`, `background`, `isolation`), `Agent(...)` allowlist, plugin-subagent security restrictions (no `hooks`/`mcpServers`/`permissionMode`).
- **Skills** — https://code.Codex.com/docs/en/skills — `SKILL.md` frontmatter (`description`, `argument-hint`, `arguments`, `disable-model-invocation`, `user-invocable`, `allowed-tools`, `model`, `effort`, `context: fork`, `agent`, `paths`), `$ARGUMENTS` + `$N` + `$name` substitutions, command/skill merge note (`.Codex/commands/foo.md` and `.Codex/skills/foo/SKILL.md` are equivalent).
